...
\foreach \col/\text in {1/$E_o\cdot P_i\cdot 2K^+$,5/$E_o\cdot 2K^+$,
                         9/$E_o\cdot 2K^+\cdot MgATP$,13/$E_i\cdot 2K^+\cdot MgATP$}{
...
\foreach \row/\text in {4/$E_i\cdot K^+\cdot MgATP$,8/$E_i\cdot MgATP$,
                         12/$E_i\cdot MgATP\cdot Na^+$,16/$E_i\cdot MgATP\cdot 2Na^+$}{
  \node[rectblue,left=2mm of right-\row-1] (right2-\row) {\text};
}

\node[rectblue,below=2mm of  right-20-1] (right2-20)  {$E_i\cdot MgATP\cdot 3Na^+$} ;
...
\node[rectred,below=\colsep of H] (MgATP)  {$q_i^{MgATP}$} ;
\node[rectred,above=\colsep of bottom-1-5] (MgADP)  {$q_i^{MgADP}$} ;
...
\draw[arrow] (bottom-1-5) -- (MgADP);
\draw[arrow] (top-1-4) |- (Pi);
\draw[arrow] (top-1-4) |- (H);
\draw[arrow] (MgATP) -| (top-1-6);
