...
\matrix (top) [matrix of nodes,column sep=\sep] {
  |[rectred]| $q_m^{4}$ & |[greencycle]| $v_m^{4}$ & |[yellowbox]| $Rx_m^{4}$ & |[greencycle]| $v_m^{4}$ & |[rectred]| $q_m^{1}$\\  
};
...
\matrix (left) [matrix of nodes,row sep=\sep, matrix anchor=north, below=\sep of top-1-1]  {
 |[greencycle]| $v_m^{3}$ \\  |[yellowbox]| $Rx_m^{3}$ \\ |[greencycle]| $v_m^{3}$ \\  |[rectred]| $q_m^{3}$  \\ 
};

\foreach \row/\text in {4/$E_o\cdot P_i$}{
...
\foreach \row/\text in { 4/$E_i\cdot P_i\cdot 3Na^+$}{
  \node[rectblue,right=2mm of right-\row-1] (right2-\row) {\text};}

\matrix (bottom) [matrix of nodes,column sep=\sep, matrix anchor=west, right=\sep of left-4-1]  {
   |[greencycle]| $v_m^{2}$ & |[yellowbox]| $Rx_m^{2}$ & |[greencycle]| $v_m^{2}$\\
};

\draw[arrow] (left-1-1) -- (top-1-1) ;
\draw[arrow] (top-1-5) -- (right-1-1);
\draw[arrow] (bottom-1-1) -- (left-4-1);
\draw[arrow] (right-4-1) -- (bottom-1-3) ;
...
\node[rectred,left=\sepp of left-3-1] (Ko)  {$q_o^{K^+}$} ;
...
\node[rectred,right=\sepp of right-3-1] (ADP)  {$q_i^{ADP}$} ;
\node[rectred,above=20mm of bottom-1-2] (um)  {$u_m^{e}$} ;
\node[bluecycle,above=\sep of um] (z2)  {\textcolor{white}{$z_2F$}} ;
\node[bluecycle,below=\sep of um] (z1)  {\textcolor{white}{$z_1F$}} ;
\draw[arrow] (z2) -- (um);
\draw[arrow] (um) -- (z1);
\draw[arrow] (top-1-4) |- (z2);
\draw[arrow]  (z1) -|(bottom-1-3);
\draw[arrow] (Ko) -- (left-3-1);
\draw[arrow] (bottom-1-1) |- (Nao);
\draw[arrow] (top-1-4) |- (Ki);
\draw[arrow] (Nai) -- (right-1-1);
\draw[arrow] (right-3-1) -- (ADP);
...
\begin{scope}[on background layer]
  \node[fit=(top-1-1)(right-4-1), fill=yellow!20, inner sep=6pt] (fitnode) {};
