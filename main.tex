...
We then mainly focus on how the bond graph facilitates analysis to examine the energy expenditure of the \Gls{nka} and energy storage and dissipation under different conditions.
...
We highlight energetic profiles of \gls{nka} under physiological and pathophysiological conditions can be used for model validation, model comparison and model selection for different applications.
...
While these structural studies have provided insights into the molecular mechanism of the \Gls{nka}, Wagoner et al. found out that \gls{nka} achieved high speed and efficiency by ustilizing theses multiple conformational states \cite{wagoner_mechanisms_2019}.
...
Many kinetic \Gls{nka} models including varied number ofintermediate conformational states and partial reaction steps have been proposed \cite{post_flexibility_1969,karlish_conformational_1978,glynn_na_1985, apell_electrogenic_1989,terkildsen_balance_2007,guerra_multistate_2025}.
...
It is important to guarantee thermodynamic consistency in \Gls{nka} model development. Hence, the bond graph is also used in this study, and more importantly we demonstrate how the bond graph facilitates energy-based analysis.
...
\begin{itemize}
      ...
      model composition \cite{shahidi_hierarchical_2021,shahidi_sbml_2022,safaei_bond_2018}, SLC transporters BJ paper\cite{hunter_energy-based_2025}     
\end{itemize}
...
In this article, we use a slightly deferent notation to that used in previous bond graph papers \cite{gawthrop_energy-based_2014,pan_bond_2018,pan_bond_2019,pan_cardiac_2020,hunter_energy-based_2025} to make the bond graph more accessible to a wider audience.
The key concepts are illustrated in Figure~\ref{fig:bg_concepts}.
Lines of power transmission called \emph{bonds} always have an associated flux $v$ and potential $\mu$, and the product of flux $v$ and potential $\mu$ is power, i.e., $P=\mu\cdot v$. The arrow direction specifies the positive reference direction of the energy flow, i.e., the direction of power. In a dynamic system, the energy flow  $P(t)=\mu(t)\cdot v(t)$ direction may change over time, while a postive power $P(t)>0$ indicates the energy is released at the tail at time $t$. 

If these bonds meet, the sum of powers must be zero $\sum \mu\cdot v=0$, to ensure power conservation. If they share a common potential $\mu$ (called a \emph{0 node}), power conservation becomes just $\sum v=0$ (for non-zero u), which is mass conservation if $v$ is a molar flux or mechanical flow and charge conservation if $v$ is an electrical flux. Alternatively, if they share a common flux $v$ (called a \emph{1 node}), power conservation  becomes just $\sum \mu=0$ (for non-zero v), which is energy conservation.  

\begin{figure}[H]  
\centering  
\input{Fig1a.tex}
\caption{(a) a bond, which transmits energy, carries a flow $v_j$ and a potential $\mu_j$;
...
(e) a concise way to be more succinctly expressed by the red-bordered box where the potential $\mu_c^1$ is given by an empirically defined capacitive storage relationship $\mu_c^1=f(q_c^1)$.}
\label{fig:bg_concepts}
\end{figure}

The explicity expression of energy flows in bond graphs makes them particularly suitable for thermodynamic analysis of biochemical systems. For each component $j$ in the bond graph, the energy flow can be calculated as the sum of the power transmitted by each of its bonds, that is $P_j=\sum_{i = 1}^{k}  \mu_j^i \cdot v_j^i$ with $i=1,2,\ldots,k$ indicating the number of bonds connected to the component. For dynamic systems, the power can be integrated over time to give the total energy stored or dissipated by the component over the time interval $t_0$ to $t_1$, that is $E_j=\int_{t_0}^{t_1} P_j(t) \mathrm{d}t$.
...
\begin{equation}
      ...
      \Delta G_{ATP} =  \mu_{ADP} + \mu_{P_i} + \mu_{H^+} - \mu_{ATP}
\end{equation}
where $\mu_{ATP}$, $\mu_{ADP}$, $\mu_{P_i}$, and $\mu_{H^+}$ are the chemical potentials of \gls{atp}, \gls{adp}, \gls{pi}, and protons $H^+$, respectively.
...
\begin{equation}
      ...
      \eta_{est} = \frac{\left(3\mu_o^{Na^+} + 2\mu_i^{K^+}-3\mu_i^{Na^+}-2\mu_o^{K^+}-FV_m\right)}{\Delta G_{ATP}} \cdot 100\%
\end{equation}

where $\mu_i^{Na+}$ and $\mu_o^{Na+}$ are the chemical potentials of intracellular and extracellular Na\textsuperscript{+}, respectively,
$\mu_i^{K+}$ and $\mu_o^{K+}$ are the chemical potentials of intracellular and extracellular K\textsuperscript{+}, respectively,
...
\begin{equation}
      ...
      E_{supply} = \int_{0}^{t} \mu_{ATP} \cdot v_{ATP} \mathrm{d}t- \int_{0}^{t} \mu_{ADP} \cdot v_{ADP} \mathrm{d}t - \int_{0}^{t} \mu_{P_i} \cdot v_{P_i} \mathrm{d}t - \int_{0}^{t} \mu_{H^+} \cdot v_{H^+} \mathrm{d}t
\end{equation}
...
\begin{equation}
      ...
      E_{storage}^{Na^+} = \int_{0}^{t}\mu_o^{Na+} \cdot v_o^{Na+} \mathrm{d}t- \int_{0}^{t} \mu_i^{Na+} \cdot v_i^{Na+}  \mathrm{d}t 
\end{equation}
\begin{equation}
      \label{eq:energy_storage_K}
      E_{storage}^{K^+} =  \int_{0}^{t}\mu_i^{K+} \cdot v_i^{K+} \mathrm{d}t- \int_{0}^{t} \mu_o^{K+} \cdot v_o^{K+}  \mathrm{d}t 
\end{equation}
...
\begin{equation}  
      ...
      E_{storage}^{electrical} = \int_{0}^{t} V_m \cdot i_{m} \mathrm{d}t
\end{equation}
...
while the recent study showed lower diastolic cytosolic \gls{atp} levels around 457 $\pm 47\mu M$ (less than 1 mM) than previously estimated and fluctuations during excitation-contraction coupling \cite{rhana_fueling_2024}.

The cytosolic \gls{adp} concentrations of in situ mouse, rat and guinea pig heart is 13, 18 and 22 $\mu M$ \cite{dobson_heart_2002}, respectively, while the extrapolated value in human heart is 45.66 $\mu M$  with 70 kg body weight.
...
The model simulation reported the \gls{atp}, \gls{adp}, \gls{pi} and pH values of 5.61 mM, 25.4 $\mu M$, 1.16 mM and 7.15 in \cite{tran_regulation_2015}. In general, the physiological intracellular pH ranges from 7.15 to 7.25 \cite{orlowski_intracellular_2025}

Intracellular Na\textsuperscript{+} concentration is normally from 4 to 16 mM \cite{bers_intracellular_2003}, while the level can rise to 3 mM higher in heart failure conditions \cite{despa_intracellular_2002}. This increase was due to increased Na\textsuperscript{+} influx while the \gls{nka} function is unchanged \cite{despa_intracellular_2002}.
