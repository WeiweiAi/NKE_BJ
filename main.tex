...
While these structural studies have provided insights into the molecular mechanism of \Gls{nka}, Wagoner et al. \cite{wagoner_mechanisms_2019} discovered that \gls{nka} achieved high speed and efficiency by ustilizing these multiple conformational states.
...
\end{equation}
The efficiency defined in Equation \ref{eq:efficiency} is different from the efficiency estimated by Equation \ref{eq:efficiency_estimate}, which is based on the steady state assumption and does not account for the time-varying nature of the membrane potential and other dynamic factors. The efficiency defined in Equation \ref{eq:efficiency} provides a more comprehensive and accurate measure of the energetic efficiency of the \gls{nka} under dynamic conditions. This eficiency can be used to assess the performance of the \gls{nka} under different physiological and pathophysiological conditions, and to compare the energetic efficiency of different \gls{nka} models.

Please note that the normal range of \gls{nka} efficiency is between 0 and 100\%, where the \gls{atp} hydrolysis is the only source of energy. When the calculated efficiency is greater than 100\%, it indicates that there are other sources of energy contributing to the transport process, such as the energy stored in the concentration gradients of Na\textsuperscript{+} and K\textsuperscript{+} or the electrical energy stored in the membrane potential. In such cases, the efficiency can be interpreted as the ratio of useful energy stored to the total energy supplied from \gls{atp} hydrolysis, and it may exceed 100\% due to the additional energy contributions from other sources. At extreme conditions, the calculated efficiency can be negative, which indicates that the \gls{nka} operates in reverse mode under certain conditions, using the energy stored in the concentration gradients of Na\textsuperscript{+} and K\textsuperscript{+} or the electrical energy stored in the membrane potential to drive the synthesis of \gls{atp}. The reversal of the pump cancels out some of the forward fluxes, which results in reduced efficiency. When the accumulated $E_\text{supply}$ from the reverse fluxes is greater than the accumulated value from the forward fluxes, the value of $E_\text{supply}$ becomes negative so as the efficiency.
...
We applied a series of action potentials (with \gls{bcl} of 1000 ms) to the \gls {nka} models while keeping the Na\textsuperscript{+} and K\textsuperscript{+} concentrations constant. The action potentials were simulated using the Faber-Rudy model\cite{faber_action_2000} with corrected T type calcium current \cite{tong_computational_2014} , which were retrieved from PMR (\url{https://models.physiomeproject.org/e/260}). The other simulation conditions are shown in Table \ref{tab:default_parameters}. We then calculated the energy flows through the \gls{nka} over ten cycles of the action potential. We plot the results in Figure \ref{fig:energy_flow} (a and b). 
