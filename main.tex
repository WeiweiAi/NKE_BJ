...
The cytosolic \gls{adp} concentrations of in situ mouse, rat and guinea pig heart are 13, 18 and 22 $\mu$M \cite{dobson_heart_2002}, respectively, while the extrapolated value in human heart is 45.66 $\mu$M with 70 kg body weight.
...
The model simulation \cite{tran_regulation_2015} reported \gls{atp}, \gls{adp}, \gls{pi} and pH values of 5.61 mM, 25.4 $\mu$M, 1.16 mM and 7.15, respectively. In general, the physiological intracellular pH ranges from 7.15 to 7.25 \cite{orlowski_intracellular_2025}.
...
We applied a series of action potentials (with \gls{bcl} of 1000 ms) to the \gls{nka} models while keeping the Na\textsuperscript{+} and K\textsuperscript{+} concentrations constant. The action potentials were simulated using the Faber-Rudy model \cite{faber_action_2000} with corrected T type calcium current \cite{tong_computational_2014}, which were retrieved from PMR (\url{https://models.physiomeproject.org/e/260}). The other simulation conditions are shown in Table \ref{tab:default_parameters}. We then calculated the energy flows through the \gls{nka} over ten cycles of the action potential. We plotted the results in Figure \ref{fig:energy_flow} (a and b). 
...
We can see that the thermodynamic efficiency of both models increases with lower $\Delta G_{ATP}$ values (less negative) until a drop in full bond graph models after the $\Delta G_{ATP}$ reaches around -48 kJ/mol. 
...
Since Equation \ref{eq:deltaG_nka} does not account for the dynamics of the \gls{nka}, it cannot explain the differences between the 15-state and 6-state models or the drop at different \gls{bcl} values. To investigate the mechanism behind the drop in thermodynamic efficiency at low $\Delta G_{ATP}$ values, we computed biochemical conversions efficiency $\eta_{ch}$ and electrical conversion efficiency $\eta_{e}$ using equations \ref{eq:chemical_efficiency} and \ref{eq:electrical_efficiency}, respectively. Both $\eta_{ch}$ and $\eta_{e}$ contribute to the overall thermodynamic efficiency drop, while the \gls{bcl} of action potential has more obvious effect on $\eta_{e}$, as shown in Figure \ref{fig:electrical_efficiency_AP_deltaGATP}.
...
With varied \gls{adp}, the electrical conversion efficiency of 15-state model starts to decrease earlier than the 6-state model;
