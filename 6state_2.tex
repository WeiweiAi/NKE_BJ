...
\foreach \row/\text in {4/$E_i\cdot ATP$, 8/$E_i\cdot ATP\cdot 3Na^+$}{
...
\node[rectred,right=\sepp of right-1-1] (ATP)  {$q_i^{ATP}$} ;
\node[rectred,right=\sepp of right-5-1] (Nai)   {$q_i^{Na^+}$} ;
\node[bluecycle,left=2mm of Nai] (z3)   {\textcolor{white}{$3$}} ;
\node[rectred,above=16mm of ATP] (Ki)  {$q_i^{K^+}$} ;
...
\node[rectred,above=\sep of bottom-1-2] (ADP)  {$q_i^{ADP}$} ;
...
\draw[arrow] (Nai) -- (right-5-1);
\draw[arrow] (bottom-1-1) -- (ADP);
