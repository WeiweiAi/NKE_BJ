...
\foreach \row/\text in {4/$E_i\cdot MgATP\cdot 3Na^+$, 8/$E_i\cdot P_i\cdot 3Na^+$}{
...
\node[rectred,below=\sep of Nai] (MgATP)  {$q_i^{MgATP}$} ;
\node[rectred,right=\sepp of right-7-1] (MgADP)  {$q_i^{MgADP}$} ;
...
\draw[arrow] (right-7-1) -- (MgADP);
\draw[arrow] (left-1-1) -- (Pi);
\draw[arrow] (left-1-1) -- (H);
\draw[arrow] (MgATP) -- (right-1-1);
...
% Right band: from fitnode.south east to fitnode.north east shifted right by \bandwidth
    \fill[LimeGreen!80!gray!30, rounded corners=2pt]
      (fitnode.south east) rectangle ($ (fitnode.north east) + (33mm,0) $);
