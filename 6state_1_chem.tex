...
\matrix (right) [matrix of nodes,row sep=\sep, matrix anchor=north, below=\sep of top-1-2]  {
  |[rectblue]| $E_i\cdot MgATP\cdot 3Na^+$ \\ 
...
\node [rectblue, right=of top-1-2, xshift=5mm,yshift=-10mm] (MgATP) {$ MgATP$};
\node [rectblue, right=of top-1-2, xshift=5mm,yshift=-15mm] (Na) {$ 3Na^+$};
\draw[bend right]  (MgATP) to ([xshift=1mm,yshift=5mm]right-1-1.north);
\draw[bend right] (Na) to ([xshift=1mm,yshift=5mm]right-1-1.north);

\node [rectblue, left=of right-2-1, yshift=12mm] (MgADP) {$ MgADP$};
\draw[bend right] (MgADP) to ([xshift=-1mm,yshift=-5mm]right-1-1.south);

\node [rectblue, right=of left-2-1, yshift=8mm] (Na_o) {$ 3Na^+$};
